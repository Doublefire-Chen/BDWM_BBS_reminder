...
def main():
	...
	driver.get('https://bbs.pku.edu.cn/v2/mobile/home.php')
	driver.delete_all_cookies() # 删除第一次建立连接时的cookie
	cookie1={"name" : "skey","value" : skey} #cookie变量
	cookie2={"name" : "uid","value" : uid} #cookie变量
	driver.add_cookie(cookie1)	#添加cookie
	driver.add_cookie(cookie2)	#添加cookie
	driver.get('https://bbs.pku.edu.cn/v2/mobile/home.php') #爬首页
	driver.refresh() #刷新
	print("连接成功") #提示性输出
	soup=BeautifulSoup(driver.page_source,"html.parser") #解析
	if Big_ten_flag=="0":
		Big_ten='************全站十大************\n' #初始赋值
	...
	if mail_num!=None:  #爬站内信
		...
		driver.get('https://bbs.pku.edu.cn/v2/mobile/mail.php') #爬站内信网页
		driver.refresh() #刷新
		soup=BeautifulSoup(driver.page_source,"html.parser") #解析
		mail='************新站内信************'+'\n' #初始赋值
	...
	if message_num!=None:  #爬消息
		...
		driver.get('https://bbs.pku.edu.cn/v2/mobile/message.php') #爬消息网页
		driver.refresh() #刷新
		soup=BeautifulSoup(driver.page_source,"html.parser") #解析
		message="************新消息************"+'\n' #初始赋值
